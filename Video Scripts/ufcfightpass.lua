...
-- https://ufcfightpass.com/live/129428/fight-pass-247
	...
	local url = 'https://dce-frontoffice.imggaming.com/api/v2/'
	local function showMsg(str)
		local t = {text = 'UFC Fight Pass: ' .. str, color = ARGB(255, 255, 102, 0), showTime = 1000 * 5, id = 'channelName'}
		m_simpleTV.OSD.ShowMessageT(t)
	end
	local function GetTokens()
		...
		local authorisationToken = answer:match('"authorisationToken":"([^"]+)')
			if not authorisationToken then return end
	 return authorisationToken
	end
	local function GetAddress(id, token)
		local headers = headers .. token
		local url = url .. 'stream?sportId=0&propertyId=0&tournamentId=0&displayGeoblockedLive=false&eventId=' .. id
		local rc, answer = m_simpleTV.Http.Request(session, {url = url, headers = headers})
			if rc ~= 200 then return end
		local url = answer:match('"playerUrlCallback":"([^"]+)')
	...
	local retAdr = GetAddress(id, authToken)
		if not retAdr then
			showMsg('нет адреса трансляции')
		 return
		end
