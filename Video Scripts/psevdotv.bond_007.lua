-- видеоскрипт "Бонд 007" [псевдо тв] https://wink.rt.ru (7/3/21)
-- Copyright © 2017-2021 Nexterr | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- скрапер TVS: psevdotv_pls.lua
-- видоскрипт: videocdn.lua, YT.lua
-- ## открывает ссылку ##
-- https://psevdotv.bond_007
-- ##
		if m_simpleTV.Control.ChangeAddress ~= 'No' then return end
		if not m_simpleTV.Control.CurrentAddress:match('^https?://psevdotv%.bond_007') then return end
	local inAdr = m_simpleTV.Control.CurrentAddress
	if m_simpleTV.Control.MainMode == 0 then
		m_simpleTV.Interface.SetBackground({BackColor = 0, TypeBackColor = 0, PictFileName = '', UseLogo = 0, Once = 1})
		if m_simpleTV.Control.ChannelID == 268435455 then
			m_simpleTV.Control.ChangeChannelLogo('https://raw.githubusercontent.com/Nexterr-origin/simpleTV-Images/main/bond_007.png', m_simpleTV.Control.ChannelID)
		end
	end
	...
	local pls = decode64('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL05leHRlcnItb3JpZ2luL3NpbXBsZVRWLVBsYXlsaXN0cy9tYWluLzAwNw')
	...
	local title, adr
		for w in answer:gmatch('#EXTINF:(.-\n.-)%c') do
			...
			if adr and title then
				tab[i] = {}
				tab[i].Id = i
				tab[i].Address = adr .. '&isLogo=false&isPlst=true$OPT:INT-SCRIPT-PARAMS=psevdotv'
				tab[i].Name = title
				i = i + 1
			end
		end
