-- видеоскрипт для плейлиста "wifire" https://wifire.tv (5/5/20)
...
-- ##
	...
	local extOpt = '$OPT:no-ts-trust-pcr$OPT:adaptive-use-access$OPT:http-user-agent=' .. userAgent
	local retAdr = 'http://api.wifire.tv/proxy/cookies?url=http%3A%2F%2Fsgw.tv.ti.ru%2FstreamingGateway%2FGetLivePlayList%3Fsource%3D'
	...
	local t = {}
		for w in answer:gmatch('EXT%-X%-STREAM%-INF(.-\n.-)\n') do
			...
			if adr and bw then
				...
				t[#t].Address = adr:gsub('https', 'http') .. extOpt
			end
		end
		if #t == 0 then
	...
	local lastQuality = tonumber(m_simpleTV.Config.GetValue('wifire_qlty') or 100000000)
	t[#t + 1] = {}
	t[#t].Id = 100000000
