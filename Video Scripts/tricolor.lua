-- видеоскрипт для плейлиста "Триколор ТВ" https://tricolor.ru (10/10/25)
...
-- http://nea-live-stream.ott.tricolor.tv/streamingGateway/GetLivePlayList?source=domashny.m3u8
	...
	local function GetToken()
		...
		if saveToken and CheckToken(saveToken) == 200 then
			tok = saveToken
		elseif saveToken and CheckToken(saveToken) ~= 200 then
			showMsg(CheckToken(saveToken), ARGB(255,255, 0, 0))
		elseif not saveToken then
