-- скрапер TVS для загрузки плейлиста "voka" https://www.voka.tv (12/6/23)
...
local filter = {
	...
	local function LoadFromSite()
		...
		local url = decode64('')
			...
			while tab.data[i] do
				...
				then
					...
					if tab.data[i].catchup_availability.period.unit == 'hours' then
						period = period * 60
						t[#t].RawM3UString = 'catchup="append" catchup-minutes="' .. period
				...
				t[#t].name = tab.data[i].name:gsub('%(новый%)', ''):gsub('%(тест%)', ''):gsub('%(Тест%)', ''):gsub('%(тест, англ%.%)', '')
				t[#t].address = 'https://www.voka.tv/' .. tab.data[i].id
				t[#t].group = tab.data[i].genres[1].name
				i = i + 1
			end
	 return t
	end
