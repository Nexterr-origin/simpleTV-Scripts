-- скрапер TVS для загрузки плейлиста "Salom TV" https://salomtv.uz (15/8/24)
-- Copyright © 2017-2024 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- видеоскрипт: salomtv.lua
-- ## Переименовать каналы ##
local filter = {
	{'Setanta Sports Plus', 'Setanta Sports+'},
	{'Евроспорт 2', 'Eurosport 2'},
	}
	local host = 'https://salomtv.uz/'
	local my_src_name = 'Salom TV'
	module('salomtv_pls', package.seeall)
	...
	function GetSettings()
	 return {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\salomtv.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 1, RefreshButton = 1, show_progress = 0, AutoBuild = 0, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, LogoTVG = 0}, STV = {add = 1, ExtFilter = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, HDGroup = 1, AutoSearch = 1, AutoNumber = 1, NumberM3U = 0, GetSettings = 1, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 0, RemoveDupCH = 1}}
	end
	function GetVersion()
	 return 2, 'UTF-8'
	end
	local function LoadFromSite()
		...
		local t = {}
			for i = 1, #tab.tv_channels do
				local slug = tab.tv_channels[i].flusonic_id
				slug = string.lower(slug)
				local id = tab.tv_channels[i].id
				local title = tab.tv_channels[i].title_ru
				if slug and id and title then
					t[#t + 1] = {}
					t[#t].name = unescape3(title)
					t[#t].address = host .. slug .. '/' .. id
					t[#t].logo = tab.tv_channels[i].image or ''
				end
			end
	 return t
	end
